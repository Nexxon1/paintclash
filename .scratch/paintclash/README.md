...
6. **Jede Raum-Erstellung *und jeder Socket* kommt von ihrer eigenen Adresse** (Erstellung seit Ticket 14, Sockets seit Ticket 15). `POST /api/rooms` ist pro IP raten-begrenzt (spec §8.3 Punkt 6) und **jeder `/ws`-Aufbau ebenfalls** (Punkt 3: Join-Rate + gleichzeitige Sockets pro Adresse); `room.test.ts` erstellt und betritt weit mehr Räume als eine Adresse darf. Teilten sie sich eine, würde irgendein Test mit `429` fallen — aus einem Grund, der nichts mit dem zu tun hat, was er prüft. Jede Datei hat darum ein `freshCaller()` und setzt `CF-Connecting-IP` pro Socket. Die Ausnahmen sind die Tests, deren *Subjekt* eine Adresse ist (Raum-Rate-Limit in `room.test.ts`, die Pro-IP-Deckel in `abuse.test.ts`): die pinnen ihre Adresse absichtlich und weisen am Ende nach, dass eine *andere* unberührt bleibt.
...
8. **Eine Choreografie, an der man nicht einzeln iterieren kann, ist ein Problem für sich** (seit Ticket 27). Hängt ein Test an dem Arena-Zustand, den die Tests vor ihm hinterlassen, fällt er gefiltert *immer* — und dann lässt sich weder bisecten noch messen. Braucht eine Choreografie einen Zustand für sich, bekommt sie ihren **eigenen privaten Raum** (= eigenes Arena-DO, ADR-0004; `soloRoom()` in `death.test.ts`), nicht einen Platz in der geteilten Arena. Und wo eine Prämisse gerechnet wird, gehört die Rechnung als **reine Funktion** nach `lib/` mit eigenen Unit-Tests (`lib/wall-slide.ts`): dort ist ein Messfehler in Millisekunden zu sehen statt in einem 20-s-workerd-Lauf. `lib/` ist aus dieser Suite ausgeklammert und läuft im Root-Vitest-Lauf mit — ein Szenario-Test ist laut CONTEXT.md stack-durchgreifend, Arithmetik ist es nicht.

...
**2026-07-20 (Abschluss):** User-Abnahme: „lange nicht mehr so schlimm wie anfangs" — Restwahrnehmung (gelegentliches leichtes Beschleunigen) ist das *bewusste* Aufholverhalten der Glättung (Glide ≤ +5 WU/s eigen, Servo ≤ 2× fremd) und kein Fehler; weiteres Feintuning hat abnehmenden Ertrag und wird sinnvoller mit echtem Gameplay (Trails ab T04 geben visuelle Anker, T07-Rewind adressiert Fairness, T16 misst produktiv). Klarstellung Maßstab: **1 Gitter-Quadrat = 10 WU**; Sichten-Versatz eigen↔fremd ≈ 0,5 s (≈ halbes Quadrat), Worst Case mit voll adaptiertem Puffer ≈ 0,9 s — Logik-Konsequenzen sind in Ticket 05 als Rewind-Anforderung festgehalten.

**Bilanz Ticket 03 gesamt:** Walking Skeleton end-to-end + sechs Härtungsrunden Netcode/Rendering (Input-Queue & Jitter-Puffer, Hibernation/Idle-Sweep gegen Geister, Render-Uhr mit Servo & adaptivem Puffer, pop-freie Reconciliation mit Tempo-gekapptem Glide, Sim/Render-Entkopplung, stallfester Server-Ticker, Render-Grenz-Guards). Absicherung: 133 Unit/Property/Replay, 5 Szenario, 4 E2E (inkl. Stall-Recovery- und Smoothness-Wächter), 60-s-Soak (`pnpm soak [url]`) lokal + Produktion PASS. Deployed: https://paintclash.secure-data.workers.dev.

**2026-07-20 (Nachtrag — Bug-Jagd über den Gesamt-Diff, auf User-Wunsch):** Vier parallele Review-Agenten (Linsen: Client-Netcode, Server/DO-Lebenszyklus, Sim-Core/Protokoll, Integration) über `59be2a9..HEAD`; 12 echte Funde verifiziert und gefixt (Commit `6c76be5`):

- **HIGH · 256-Spieler-Freeze:** kein Populations-Limit + u8-Zählbyte im Snapshot → ab 256 Spielern waren ALLE Snapshots für ALLE Clients undekodierbar (billiger Global-Freeze-DoS). Fix: harter Cap `LIMITS.maxConnections = 64` (provisorisch aus Bench-Ticket 02; T15 setzt den Gameplay-Wert), Ablehnung mit Close 1013 „arena full".
- **HIGH · Unsterblicher Geist:** Join+Disconnect im selben Tick (Leaves laufen vor Joins) spawnte einen Spieler ohne Verbindung — für immer. Fix: `disconnect()` storniert ungespawnte Joins.
- **HIGH · Gestapelte Doppel-Session:** erneuter „Spielen"-Klick nach Disconnect ließ alten rAF-Loop/Szene/Timer weiterlaufen (Canvas-Kampf, toter Input-Konsument). Fix: vollständiger Teardown vor jedem Start.
- MED: Snapshots nur noch an gejointe Sockets (§8.2); ID-Recycling statt monotonem Zähler (u16-Wire-Grenze); Adaptive-Delay wächst nur bei fließenden Daten (Totalausfall pumpte ihn ans Maximum) + Rückwärts-Hard-Resync (Gegner froren nach Netzausfall sekundenlang); Soak-Reset-Detektor dt-normalisiert (war im Stall-Frame blind); Join-Decoder erzwingt 16-Code-Point-Namensgrenze.
- LOW: Spawn-Clamp für Mini-Arenen; socketIds-Aufräumen bei server-seitigem Kill; Respawn-Klasse-Sprünge snappen Heading mit; E2E-Polls lesen `lastRender` statt mutierendem `renderSample()`.

Bewusst nicht gefixt: Wiederholtes `welcome` auf derselben Verbindung resettet die Session nur teilweise — mit dem aktuellen Server unerreichbar; vorgemerkt für ein späteres Reconnect-Feature. Endstand: 136 Unit (4 neue), 5 Szenario, 4 E2E, Soak lokal + Produktion PASS; deployed `6c76be5`.

**2026-07-20 (Nachtrag — Sichten-Versatz quantifiziert & Backlog-Pinning gefixt):** User-Beobachtung „~1 Quadrat (=10 WU) Unterschied zwischen zwei Tabs". Befund: (1) Geister aus Vor-Fix-Builds verfälschten den Eindruck (mit aktuellem Build nicht mehr reproduzierbar — Repro-Skript bestätigt saubere Arena nach abruptem Browser-Tod). (2) Echter Bug: **stehender Input-Backlog** — nach jedem Burst blieb die Queue dauerhaft bei 6 Ticks (Zufluss=Abfluss=1/Tick) = +300 ms permanente Input-Latenz pro Spieler; bei zwei Sichten summiert ≈ +5,4 WU → erklärte das Quadrat. Fix: sanfter Trim (1 Extra-Intent nach 2 s über Ziel 3) zusätzlich zum Burst-Drain; Unit-Test nagelt den Trim fest. **Gemessene Werte danach** (Sonde `tests/soak/offset-probe.mjs`): Einzelsicht-Versatz p50 **1,7 WU lokal / 2,5 WU Produktion (0,19/0,27 s)**; Nebeneinander-Diskrepanz = Summe beider Sichten ≈ **5 WU ≈ halbes Quadrat** — Genre-Normalbereich. Rest ist Physik (Prediction-Vorsprung + Interpolations-Vergangenheit) und wird für Kill-Fairness vom Rewind (T05/07) behandelt.

**2026-07-20 (Korrektur — Sichten-Versatz ehrlich gemessen):** User-Screenshot (2 Fenster nebeneinander) widerlegte die 2,5-WU-Messung: Die Sonde ließ den Spieler **kreisen**, wodurch der Zeitversatz nur als Sehne des 1,6-WU-Kreises erschien — Messfehler, der den echten Wert massiv unterschätzte. Sonde auf **Geradeausfahrt im freien Feld** umgebaut (`tests/soak/offset-probe.mjs`, Wand-Ausweichen eingebaut). Ehrliche Zahlen gegen Produktion: **vor Feintuning 2,9 WU je Sicht (0,32 s) = 5,8 WU Zwei-Screens-Diskrepanz**; nach Feintuning (Interp-Basis 3→2 Ticks, stehender Backlog 3→2; Soak-gewacht: Gegner-sd 0,75, 0 Frozen) **2,6 WU je Sicht = 5,2 WU (~halbes Quadrat)**. Struktureller Boden mit dieser Architektur ≈ 4,5–5 WU (Batch-Budget ADR-0001 + Jitter-Puffer + Tick). Der 1-Quadrat-Wert des Users war real (Backlog-Pinning + Vor-Fix-Geister); Restversatz = Genre-Physik → Rewind (T05/07).

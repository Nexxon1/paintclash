...
**2026-07-21 — Nachschärfung nach weiterem User-Feedback (Smoothness, Winkel-Gap, Trail über Fremdgebiet):**

- **Spitzen-Zittern** („ganz smooth ist es immer noch nicht"): das 0,1-WU-Gate quantisierte auch die Band*spitze* — bei hohen Bildraten blieb sie bis zu 0,1 WU hinter dem Kopf und schnappte nach. Jetzt gilt: gespeicherte Punkte bleiben gegated (stabil, sägezahnfrei), aber der letzte Punkt ist **immer die Live-Kopf-Pose** (nie gespeichert; alle gespeicherten Punkte liegen hinter dem Kopf ⇒ kein Zurückfalten möglich). Messung: max. Spitze↔Kopf-Abstand über 90 Kurven-Frames = **0,0000 WU**.
- **Winkel-abhängiges Exit-Gap:** eine einzelne Innen-Pose als Seed reichte bei schrägen Ausfahrten nicht (Keil zwischen flacher Band-Startkappe und Plateaukante). Frische Trails werden jetzt **rückwärts entlang des real gerenderten Pfads** gesät, solange die Punkte im Gebiet liegen (≤ 1,5 WU Pfadtiefe) — Start liegt für jeden Winkel satt unter dem Plateau. Gleiches Verfahren für Gegner-Trails (Ring der letzten Snapshot-Posen).
- **Trail über fremdem Gebiet** (User: „man soll sich durchfressen wie bei paper.io — späteres Ticket?"): Ja — die echte **Carve-Rinne** (Fremdgebiet sinkt unter der Linie ab, Spec §4.1) gehört zu **Ticket 06** (gesamte Fremdgebiet-Interaktion). Interim ab jetzt: Trail-Punkte über fremdem Land werden markiert (`lift`) und **oben auf dem Plateau** gezeichnet (Rampe an der Kante) — die Linie ist beim Durchfahren durchgehend sichtbar statt unterm Plateau zu verschwinden. Visuell verifiziert (Screenshot: blauer Trail quer über grünem Block).

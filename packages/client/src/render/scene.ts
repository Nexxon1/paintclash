...
export class ArenaScene {
  ...
  private updateTerritories(state: RenderState): void {
    ...
    // An undrawn ribbon cuts no groove either (ticket 20) — a channel through
    // a plateau with no band running in it reads as a bug, not as polish.
   ...
   * circling on ground you already own) is treated exactly like no trail at
   * all, down to disposing its ribbon: the player is still cuttable on it, it
   * is simply not worth a band on screen.
   */

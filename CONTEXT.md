...
- **Jitter-Puffer** — server-seitiges Anhalten der Input-Queue um wenige Ticks (`LIMITS.inputBufferTicks`), damit ein leicht verspäteter Input-Batch die Queue nie leerlaufen lässt (Leerlauf verschiebt die Input-Zeitachse und wird beim Client als Reconciliation-Ruckler sichtbar).

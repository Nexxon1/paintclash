...
- **Tod** — ausgelöst durch (1) Trail-Schnitt (fremd oder selbst), (2) Kopf-an-Kopf während man draussen ist, (3) Totalverlust des Gebiets. Folge: gesamtes Gebiet wird neutral. Auflösung pro Tick (Ticket 05): erst alle Trail-Schnitte simultan, dann Kopf-an-Kopf unter den nicht Geschnittenen (wer jemanden auf dessen Linie stellt, stirbt nicht am Kopf des frisch Geschnittenen); danach Respawn auf frischem Startblock im selben Tick. Code: `detectDeaths`/`Death` in `sim-core`.
